...
import { config, custom, saveCustomFile } from "../module/config.js";
import { printHotKey } from "./hotkey.js";
import { getEditors, setBlockDOMAttrs } from "./dom.js";
...
function createToolbarItem(toolbarConfig, className) {
	...
	if (
		...
		label += toolbarConfig.hotkey
			? ` [${printHotKey(toolbarConfig.hotkey())}]`
			: "";
	}
	...
	if (window.theme.clientMode !== "mobile" && toolbar && windowControls) {
		if (!custom_toolbar) {
			...
			more.addEventListener("click", () => {
				let status,
					language = window.theme.languageMode;
				...
				more.setAttribute(
					"aria-label",
					status.label[language] || status.label.other
				);
...
const svgClassList = [null, "ft__primary", "ft__secondary"];
			...
			case "SVG":
				if (active !== null && svgClassIndex > 0) {
					if (active) {
						...
						node.firstElementChild.classList.remove(
							svgClassList[svgClassIndex]
						);
					}
 ...
 * 创建右键菜单项图标
 */
function createMenuItemIconNode(
	href = "#",
	style = "",
	className = "b3-menu__icon"
) {
 ...
 * 创建右键菜单项输入框
 */
function createMenuItemInputNode(
	id = null,
	placeholder = "",
	value = "",
	style = "",
	className = "b3-text-field fn__size200"
) {
 ...
 * 创建右键菜单项标签
 */
function createMenuItemLabelNode(
	content = "菜单项",
	style = null,
 ...
 * 创建右键菜单项捷径标签
 */
function createMenuItemAcceleratorNode(
	content,
 ...
 * 创建右键菜单项子菜单图标
 */
function createMenuItemSubMenuIconNode(
	href,
	style = null,
	className = "b3-menu__icon b3-menu__icon--arrow"
) {
...
const TASK_HANDLER = {
	...
	"attr-replace": async (e, id, params) => {
		...
		for (let attr of Object.keys(params)) {
			if (old_attrs[attr]) {
				// 如果属性不为空, 添加到末尾
				new_attrs[attr] = old_attrs[attr].replace(
					params[attr].regexp,
					params[attr].substr
				);
			}
	...
	"attr-set": async (e, id, params) => {
		...
		for (let attr of Object.keys(params)) {
			if (old_attrs[attr]) {
				...
				new_attrs[attr] =
					old_attrs[attr].replace(params[attr].regexp, "") + params[attr].value;
			} else {
				// 如果属性不存在, 则直接插入
	...
	"menu-unfold": async (e, id, params) => {
		let menuItem = window.siyuan.menus.menu.element.querySelector(
			`#${params.id}`
		);
		...
		if (params.src) {
			...
			if (BLOCK) {
				...
				window.theme.openNewWindow(
					...
					BLOCK.type === "widget" ? { id: id } : undefined,
	...
	"tab-archive": async (e, id, params) => {
		const editors = getEditors().filter((editor) => {
			if (editor.protyle.model.headElement.classList.contains("item--pin"))
				return false;
			if (
				!params.unupdate &&
				editor.protyle.model.headElement.classList.contains("item--unupdate")
			)
				return false;
			return true;
		});
		if (editors.length > 0) {
			...
			editors.forEach((editor) =>
				editor.protyle.model.headElement.lastElementChild.click()
			);
	...
	switch (config.mode.toLowerCase()) {
		...
		case "input":
			...
			if (config.click.enable) {
				if (config.click.callback)
					node.addEventListener(
						"click",
						async (e) => await config.click.callback(e, id)
					);
				else {
					let handlers = [];
					config.click.tasks.forEach((task) => {
						if (TASK_HANDLER[task.type])
							handlers.push(async (e) =>
								TASK_HANDLER[task.type](e, id, task.params)
							);
					});
		...
		case "button":
			...
			node.appendChild(
				createMenuItemLabelNode(
					config.label[language] || config.label.other,
					config.label.style
				)
			);
			let accelerator = config.accelerator;
			if (typeof accelerator === "function")
				accelerator = printHotKey(accelerator(id));
			if (typeof accelerator === "string")
				node.appendChild(createMenuItemAcceleratorNode(accelerator));
			if (config.itemsIcon) {
...
class CommonMenuObserver {
	constructor(
		...
		this.observe = (target = this.menuNode, options = this.config) =>
